...
class CreateJobRequest(BaseModel):
    """Phase 2 of the two-phase upload -- turns an already-staged upload_id
    (from POST /api/uploads) into an actual, billable job. See
    routers/uploads.py for phase 1."""

    upload_id: str
    mode: str = "paid"  # "paid" or "trial" -- see jobs_service.create_job
    declared_count: int | None = None  # required for mode="paid", ignored for "trial"
...
def create_job(
    ...
    if upload_meta is None:
        raise HTTPException(
            status_code=410,
            detail="This upload has expired or was already used. Please upload the file again.",
        )
    ...
    if payload.mode == "trial":
        ...
        if remaining <= 0:
            raise HTTPException(
                status_code=422,
                detail="Your free trial has already been used on this account. Please choose the paid option instead.",
            )
        ...
        job = jobs_service.create_job(
            db, user=current_user, client_ip=ip, declared_count=None, mode="trial", trial_limit=remaining,
        )
    ...
    job = jobs_service.create_job(db, user=current_user, client_ip=ip, declared_count=declared_count, mode="paid")
    background_tasks.add_task(jobs_service.run_job, SessionLocal, job.id, raw)
    return job.to_public_dict()
