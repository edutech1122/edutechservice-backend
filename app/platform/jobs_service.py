...
from app.platform.billing import compute_paid_price
...
def run_job(db_factory, job_id: str, pdf_bytes: bytes) -> None:
    ...
    try:
        ...
        # own tally -- see the note in app/core/config.py. Paid jobs never
        # draw on the account's free-trial allowance -- that's reserved
        # exclusively for the separate "Free Trial" button above -- so this
        # is always a full-price charge with the minimum-order floor
        # (see compute_paid_price in billing.py).
        job.price_paise = compute_paid_price(job.declared_count)
        job.free_units_applied = 0
        job.billable_count = job.declared_count
